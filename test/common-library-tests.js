    ...
    if (game.system.id === 'dnd5e') {
      describe('Library topology tests for D&D5e actors', () => {
        it('Actor inventory settings for Versatile in D&D5e with a user library', async () => {
          ...
          let torches = library.getInventory(versatile.id, "Torch");
          assert.ok(typeof torches === "number", "Count of torches has a numeric value");
          let candles = library.getInventory(versatile.id, "Candle");
          assert.ok(typeof candles === "number", "Count of candles has a numeric value");
          let lights = library.getInventory(versatile.id, "Light");
          assert.ok(typeof lights === "undefined", "Light cantrip doesn't have inventory");
          let lamps = library.getInventory(versatile.id, "Lamp");
          assert.ok(typeof lamps === "undefined", "Lamp doesn't have inventory");
          await library._presetInventory(versatile.id, "Torch", 2);
          let before = library.getInventory(versatile.id, "Torch");
          await library.decrementInventory(versatile.id, "Torch");
          let afterFirst = library.getInventory(versatile.id, "Torch");
          await library.decrementInventory(versatile.id, "Torch");
          let afterSecond = library.getInventory(versatile.id, "Torch");
          await library.decrementInventory(versatile.id, "Torch");
          let afterThird = library.getInventory(versatile.id, "Torch");
        ...
        it('Actor light sources for Lightbearer, Breaker, Empty, and Bic in D&D5e with a user library', async () => {
          ...
          let breakerSources = library.actorLightSources(breaker.id); 
          ...
          assert.equal(library.actorHasLightSource(breaker.id, "Dancing Lights"), true, "Breaker is reported as having Dancing Lights");

          let bearerSources = library.actorLightSources(bearer.id);
          ...
          assert.equal(library.actorHasLightSource(bearer.id, "Torch"), true, "Bearer is reported as having the Torch light source");

          let emptySources = library.actorLightSources(empty.id);
          assert.equal (emptySources.length, 0, "Empty truly has no known light sources");
          assert.equal(library.actorHasLightSource(empty.id, "Candle"), false, "Empty is reported as not having the candle light source");

          let everythingButSources = library.actorLightSources(everythingBut.id);
          assert.equal(everythingButSources.length, 0, "Bic has no known light sources, even though it has ways of casting light");
          assert.equal(library.actorHasLightSource(everythingBut.id, "Candle"), false, "Empty is reported as not having the candle light source");
        });
      });
    }
  });
